...
// Simple RSS feed finder, looks only for "application/rss+xml" link attributes from HTML page
// source
public class RSSFeedFinder {
  ...
  public List<RSSFeedSource> parseHtmlPageForSources(@NotNull final String page) {
    ...
    for (int i = 0; i < nodes.getLength(); i++) {
      Node n = nodes.item(i);
      if (n.hasAttributes()
          && n.getAttributes().getNamedItem(TYPE_ATTR) != null
          && n.getAttributes()
              .getNamedItem(TYPE_ATTR)
              .getNodeValue()
              .trim()
              .equalsIgnoreCase(RSS_TYPE)) {
        String linkUrl = null;
        String name = "";
  ...
  public List<RSSFeedSource> findRSSSources(@NotNull final String url) {
